...
def percept_at(location) -> Message:
    """Build a valid sensor percept at the given location.

    Returns:
        A message whose only meaningful properties here are its location and
        use_state.
...
class AttentionSystemGridTest(unittest.TestCase):
    ...
    def test_an_update_adds_to_the_grid_rather_than_replacing_it(self) -> None:
        # Weight 2 so the near voxel survives the decay tick of the second update.
...
class AttentionSystemWeightTest(unittest.TestCase):
    ...
    def test_updating_with_nothing_still_decays_the_grid(self) -> None:
        self.observe_near()
        self.system.update_regions([])
        self.assertEqual(weights_by_voxel(self.system)[NEAR_VOXEL], 2.0)
...
class AttentionSystemFilterGoalsTest(unittest.TestCase):
    """filter_goals returns only the goals that live in the current grid."""
    ...
    def test_goals_without_a_location_pass_through(self) -> None:
        ...
        self.system.update_regions([region(NEAR_POINTS[0])])
        self.assertEqual(
            self.system.filter_goals([goal_at([9.0, 9, 9]), unlocated]), [unlocated]
        )
    ...
    def test_an_expired_voxel_no_longer_admits_goals(self) -> None:
        ...
        for _ in range(3):
            self.system.update_regions([region(FAR_POINT, weight=3.0)])
        # The third update decays the near voxel past its lifetime.
        self.assertEqual(self.system.filter_goals([goal]), [])


class AttentionSystemFilterPerceptsTest(unittest.TestCase):
    """filter_percepts disables percepts outside the grid, in place."""

    def setUp(self) -> None:
        self.system = AttentionSystem(voxel_size=0.01, voxel_lifetime=3)
        self.system.update_regions([region(*NEAR_POINTS)])

    def test_a_percept_inside_the_grid_keeps_its_use_state(self) -> None:
        percept = percept_at(NEAR_POINTS[0])
        (returned,) = self.system.filter_percepts([percept])
        self.assertIs(returned, percept)
        self.assertTrue(returned.use_state)

    def test_a_percept_outside_the_grid_is_disabled_but_kept(self) -> None:
        percept = percept_at([9.0, 9, 9])
        (returned,) = self.system.filter_percepts([percept])
        self.assertIs(returned, percept)
        self.assertFalse(returned.use_state)

    def test_none_percepts_pass_through(self) -> None:
        percept = percept_at([9.0, 9, 9])
        returned = self.system.filter_percepts([None, percept, None])
        self.assertEqual(returned, [None, percept, None])
        self.assertFalse(percept.use_state)

    def test_all_none_percepts_pass_through(self) -> None:
        self.assertEqual(self.system.filter_percepts([None, None]), [None, None])

    def test_percepts_pass_through_while_the_grid_is_empty(self) -> None:
        empty = AttentionSystem(voxel_size=0.01)
        percept = percept_at([9.0, 9, 9])
        self.assertEqual(empty.filter_percepts([percept]), [percept])
