...
class AttentionSystemFilterPerceptsTest(unittest.TestCase):
    """Percepts outside the grid or in inhibited voxels are disabled."""

    def setUp(self) -> None:
        self.system = AttentionSystem(voxel_size=0.01, voxel_lifetime=3)

    def test_a_percept_in_a_positive_voxel_passes_through(self) -> None:
        self.system.update_regions([region(NEAR_POINTS[0], weight=3.0)])
    ...
    def test_a_percept_outside_the_grid_is_disabled(self) -> None:
        self.system.update_regions([region(NEAR_POINTS[0], weight=3.0)])
